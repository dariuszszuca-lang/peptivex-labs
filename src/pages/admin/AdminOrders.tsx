...
const statusConfig: Record<OrderStatus, { label: string; color: string; icon: typeof Clock }> = {
  ...
  paid: { label: 'Opłacone', color: 'text-[#ea580c] bg-[#fff7ed]', icon: CheckCircle },
  ...
  return (
    <div className="p-6 max-w-6xl">
          ...
          {(['all', 'paid', 'shipped', 'delivered', 'cancelled'] as const).map(s => (
            <button
              ...
              className={`text-xs px-3 py-2 rounded-lg border transition-all ${
                statusFilter === s
                  ? 'bg-[#fff7ed] border-amber-500/30 text-[#ea580c]'
                  : 'bg-[#fafaf7] border-[#ececec] text-[#737373] hover:text-[#525252]'
              }`}
            >
      ...
      {!loading && filtered.length === 0 && !error ? (
        ...
        <div className="bg-[#fafaf7] border border-[#ececec] rounded-xl overflow-hidden">
          <table className="w-full">
            <thead>
              <tr className="border-b border-[#ececec]">
                <th className="text-left text-[#737373] text-xs uppercase tracking-wide px-4 py-3">ID</th>
                <th className="text-left text-[#737373] text-xs uppercase tracking-wide px-4 py-3">Data</th>
                <th className="text-left text-[#737373] text-xs uppercase tracking-wide px-4 py-3">Klient</th>
                <th className="text-left text-[#737373] text-xs uppercase tracking-wide px-4 py-3">Produkty</th>
                <th className="text-left text-[#737373] text-xs uppercase tracking-wide px-4 py-3">Kwota</th>
                <th className="text-left text-[#737373] text-xs uppercase tracking-wide px-4 py-3">Lang</th>
                <th className="text-left text-[#737373] text-xs uppercase tracking-wide px-4 py-3">Status</th>
                <th className="text-right text-[#737373] text-xs uppercase tracking-wide px-4 py-3">Akcje</th>
              </tr>
            </thead>
            <tbody>
              {filtered.map(order => {
      ...
      {openOrder && (
        <div className="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" onClick={() => setOpenOrder(null)}>
          <div
            ...
            className="bg-white border border-[#ececec] rounded-2xl p-6 max-w-xl w-full max-h-[90vh] overflow-y-auto"
          >
            ...
            <div className="space-y-3 text-sm">
              ...
              <div className="flex justify-between border-b border-[#ececec] pb-2">
                <span className="text-[#737373]">Kwota</span>
                <span className="text-[#ea580c] font-bold">{formatPrice(openOrder.amountTotal, openOrder.currency)}</span>
              </div>
              <div className="border-b border-[#ececec] pb-2">
                <p className="text-[#737373] text-xs uppercase tracking-wide mb-1">Produkty</p>
                {openOrder.items.map((it, i) => (
                  <div key={i} className="flex justify-between text-[#525252] text-sm">
              ...
              {openOrder.shippingAddress && (
                <div className="border-b border-[#ececec] pb-2">
                  <p className="text-[#737373] text-xs uppercase tracking-wide mb-1">Adres dostawy</p>
                  <p className="text-[#525252]">{openOrder.shippingName || openOrder.customer.name}</p>
                  <p className="text-[#525252]">{formatAddress(openOrder.shippingAddress)}</p>
                </div>
              )}

              <div className="border-b border-[#ececec] pb-3">
                <p className="text-[#737373] text-xs uppercase tracking-wide mb-2">Numer paczki (InPost)</p>
                <input
                  type="text"
              ...
              <div>
                <p className="text-[#737373] text-xs uppercase tracking-wide mb-2">Zmień status</p>
                <div className="flex gap-2 flex-wrap">
            ...
            <div className="flex gap-2 mt-6">
              ...
              <button
                ...
                className="flex-[2] bg-[#fafaf7] hover:bg-[#f5f5f0] text-[#525252] py-2.5 rounded-lg text-sm transition-colors"
              >
                Zamknij
              </button>
            </div>
